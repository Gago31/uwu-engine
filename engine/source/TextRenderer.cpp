...
int TextRenderer::init(std::string path, int font_size) {
	fontPath = path;
	fontSize = font_size;
...
void TextRenderer::renderText(ShaderPtr shader, std::string text, float x, float y, float scale, glm::vec3 color) {
	shader->use();
...
void from_json(const json& j, TextRenderer& t) {
	...
	t.fontPath = j["fontPath"];
	t.init(t.fontPath, t.fontSize);
}
